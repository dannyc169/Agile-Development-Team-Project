...
def _effective_task_owner_id(task):
    """
    Return the user who should be treated as responsible for the task.

    Assigned tasks belong to the assignee. Unassigned tasks belong to the
    creator. This matches the My Tasks and wager attribution rules.
    """
...
def join_team():
    ...
    if form.validate_on_submit():
        ...
        if team is None:
            ...
            flash(
                f"This team is full. The maximum team size is "
...
def team_detail(team_id):
    ...
    leader_username = next(
        (row.user.username for row in member_rows if row.role == "leader"),
        None,
    )
    ...
    for task in team_tasks:
        effective_user_id = _effective_task_owner_id(task)
        tasks_by_assignee_id.setdefault(effective_user_id, []).append(task)
...
def nudge_task(team_id, task_id):
    ...
    task_owner_id = _effective_task_owner_id(task)
    ...
    if not is_team_member(team.id, task_owner_id):
    ...
    recipient = db.session.get(User, task_owner_id)
...
def member_tasks(team_id, user_id):
    ...
    in_progress_tasks = [
        task for task in member_tasks_list if task.status == "in_progress"
    ]
